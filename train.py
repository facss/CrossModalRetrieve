...
if __name__ == '__main__':
	...
	with tf.Graph().as_default(), tf.Session(config=gpuconfig) as sess:	
		...
		for epoch in range(MAX_ITER):
			...
			res_epoch.append(epoch)
			res_loss.append(loss_)

		DCMH_saver=tf.train.Saver()
		DCMH_saver.save(sess,'save/model.ckpt')
	    
		print('save checkpoint done.')
		print '...training procedure finish'

		plt.figure(1)
		plt.plot(res_epoch,res_loss)
		plt.show()
        
        
	
		#test(sess,query_x,query_y, retrieval_x, retrieval_y)
